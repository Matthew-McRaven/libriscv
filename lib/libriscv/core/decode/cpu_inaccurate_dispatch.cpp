...
#define OVERFLOW_CHECKED_JUMP()                                                                                        \
  if (LIKELY(pc - exec->exec_begin() < exec->exec_end() - exec->exec_begin())) goto continue_segment;                  \
...
#include "threaded_bytecode_array.hpp"
  ...
  if (UNLIKELY(!(pc >= exec->exec_begin() && pc < exec->exec_end()))) goto new_execute_segment;
